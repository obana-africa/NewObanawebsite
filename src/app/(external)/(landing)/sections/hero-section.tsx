...
const STAT_CARDS = [
  { id: "card-1", icon: Package, value: "10,000+", label: "Metric tons of produce supplied", gradient: "linear-gradient(135deg, #1b3b5f 0%, #24507f 100%)", iconBg: "rgba(255,255,255,0.15)", delay: 0 },
  { id: "card-2", icon: Globe,   value: "20+",     label: "Brands & Partners",               gradient: "linear-gradient(135deg, #1b3b5f 0%, #24507f 100%)", iconBg: "rgba(255,255,255,0.15)", delay: 0.15 },
  { id: "card-3", icon: TrendingUp, value: "₦2B+", label: "Commerce Enabled",               gradient: "linear-gradient(135deg, #ffffff 0%, #f0f4f8 100%)", iconBg: "rgba(27,59,95,0.08)", dark: true, delay: 0.3 },
  { id: "card-4", icon: Users,   value: "5k+",     label: "Registered Businesses",           gradient: "linear-gradient(135deg, #ffffff 0%, #f0f4f8 100%)", iconBg: "rgba(27,59,95,0.08)", dark: true, delay: 0.45 },
];
...
const HeroSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    type Dot = { x: number; y: number; vx: number; vy: number; r: number; alpha: number; color: string };
    let dots: Dot[] = [];
    const COLORS = ["27,59,95","27,59,95","36,80,127","251,191,36","251,191,36"];
    const resize = () => {
      W = canvas.offsetWidth; H = canvas.offsetHeight; canvas.width = W; canvas.height = H;
      dots = Array.from({ length: DOTS }, () => ({ x: Math.random()*W, y: Math.random()*H, vx:(Math.random()-0.5)*0.45, vy:(Math.random()-0.5)*0.45, r:Math.random()*2.5+1, alpha:Math.random()*0.5+0.25, color:COLORS[Math.floor(Math.random()*COLORS.length)] }));
    };
    const draw = () => {
      ctx.clearRect(0,0,W,H);
      for (let i=0;i<dots.length;i++) for (let j=i+1;j<dots.length;j++) { const dx=dots[i].x-dots[j].x,dy=dots[i].y-dots[j].y,d=Math.sqrt(dx*dx+dy*dy); if(d<120){ctx.beginPath();ctx.strokeStyle=`rgba(27,59,95,${0.10*(1-d/120)})`;ctx.lineWidth=1.2;ctx.moveTo(dots[i].x,dots[i].y);ctx.lineTo(dots[j].x,dots[j].y);ctx.stroke();} }
      dots.forEach(d => { const g=ctx.createRadialGradient(d.x,d.y,0,d.x,d.y,d.r*2); g.addColorStop(0,`rgba(${d.color},${d.alpha*0.4})`); g.addColorStop(1,`rgba(${d.color},0)`); ctx.beginPath();ctx.arc(d.x,d.y,d.r*4,0,Math.PI*2);ctx.fillStyle=g;ctx.fill(); ctx.beginPath();ctx.arc(d.x,d.y,d.r,0,Math.PI*2);ctx.fillStyle=`rgba(${d.color},${d.alpha})`;ctx.fill(); d.x+=d.vx;d.y+=d.vy; if(d.x<0||d.x>W)d.vx*=-1; if(d.y<0||d.y>H)d.vy*=-1; });
      raf = requestAnimationFrame(draw);
    };
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden w-full bg-white" style={{ minHeight: "480px" }}>
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }} />
        <div className="bg-orb absolute top-[-80px] left-[-80px] w-[480px] h-[480px] rounded-full pointer-events-none" style={{ background:"radial-gradient(circle,rgba(27,59,95,0.18)0%,transparent 65%)",zIndex:1 }} />
        <div className="bg-orb absolute top-[15%] right-[5%] w-[300px] h-[300px] rounded-full pointer-events-none" style={{ background:"radial-gradient(circle,rgba(251,191,36,0.22)0%,transparent 65%)",zIndex:1,animationDelay:"2s" }} />
        <div className="bg-orb absolute bottom-[-40px] left-[40%] w-[360px] h-[360px] rounded-full pointer-events-none" style={{ background:"radial-gradient(circle,rgba(27,59,95,0.12)0%,transparent 65%)",zIndex:1,animationDelay:"4s" }} />

        {/* ── Main layout ── */}
        <div
          className="relative mx-auto w-full max-w-[1280px] px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-10 md:gap-12"
          style={{ zIndex:10, paddingTop:"clamp(100px,14vw,120px)", paddingBottom:"clamp(40px,6vw,60px)" }}
        >
          {/* LEFT */}
          <div className="w-full md:flex-1 md:max-w-[620px] flex flex-col items-center md:items-start text-center md:text-left">

            {/* Tag pill */}
            <div className="hero-tag inline-flex items-center gap-2 mb-5 px-4 py-2 rounded-full"
              style={{ border:"1.5px solid rgba(251,191,36,0.55)", background:"linear-gradient(90deg,rgba(27,59,95,0.08)0%,rgba(251,191,36,0.15)50%,rgba(27,59,95,0.08)100%)", boxShadow:"0 0 16px rgba(251,191,36,0.18),inset 0 0 10px rgba(251,191,36,0.06)" }}>
              <span className="text-xs font-bold uppercase tracking-[0.16em]"
                style={{ background:"linear-gradient(90deg,#1b3b5f 0%,#fbbf24 40%,#1b3b5f 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>
                SOURCING · LOGISTICS · COMMERCE · AFRICA
              </span>
            ...
            {/* Heading */}
            <h1 className="hero-heading font-extrabold leading-[1.1] mb-4 text-primary"
              style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:"clamp(2rem,5vw,3.8rem)" }}>
              The digital platform powering{" "}
              <span style={{ color:"#fbbf24",fontStyle:"italic" }}>Africa&apos;s</span>{" "}
              Fashion&amp;Beauty SMEs.
            </h1>

            {/* Description */}
            <p className="hero-desc text-primary/65 mb-7 leading-relaxed mx-auto md:mx-0"
              style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:"clamp(0.95rem,1.8vw,1.15rem)", maxWidth:"500px" }}>
              All-in-one infrastructure for businesses to source inventory, access logistics,
              reach customers, and fully digitalise commerce operations — from day one.
            </p>

            {/* Buttons */}
            <div className="hero-btns flex flex-row gap-3 items-center justify-center md:justify-start">
              <Button variant="primary" animation="ripple"
                className="bg-primary hover:bg-primary-dark text-sm px-4 py-2 md:text-base md:px-6 md:py-3 font-medium shadow-lg shadow-primary/30"
                onClick={handleExploreEcosystem}>
                Explore Ecosystem
              </Button>
              <Button variant="primary" animation="ripple"
                className="shadow-lg !text-primary bg-transparent hover:bg-primary hover:text-white transition-all duration-300 text-sm px-4 py-2 md:text-base md:px-6 md:py-3 font-medium"
                onClick={handleBecomePartner}>
                Become a Partner
              </Button>
            </div>
                ...
                return (
                  <div key={card.id} className={`stat-card-${idx} rounded-2xl p-5 flex flex-col gap-3`}
                    ...
                    <p className="text-xs font-semibold leading-snug"
                      style={{ fontFamily:"'Bricolage Grotesque',sans-serif", color:card.dark?"rgba(27,59,95,0.65)":"rgba(255,255,255,0.75)" }}>
                      {card.label}
                    </p>
                  </div>
                );
