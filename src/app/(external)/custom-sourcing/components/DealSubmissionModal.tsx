...
const DealSubmissionModal: React.FC<Props> = ({
	...
	const handleSubmit = useCallback(async () => {
		try {
			...
			// Capture reference ID if API returns one — adjust path to match backend shape
			setSubmissionRef(
				...
				<div className="px-5 py-4 border-t border-gray-100 flex-shrink-0 space-y-2">
					{step === "form" ? (
						...
						<button
							type="button"
							onClick={handleClose}
							className="w-full py-3 bg-[#1B3B5F] text-white font-semibold rounded-xl hover:bg-[#1B3B5F]/90 flex items-center justify-center gap-2 text-sm"
						>
							Done
						</button>
					)}
...
const SuccessStep: React.FC<SuccessStepProps> = memo(function SuccessStep({
	referenceId,
	email,
	contactName,
}) {
	...
	return (
		<div className="space-y-5 py-4 sm:py-6">
			...
			<div className="text-center space-y-1.5 px-4">
				<h3 className="text-lg font-bold text-[#1B3B5F]">
					{firstName ? `Thanks, ${firstName}!` : "Request submitted!"}
				</h3>
				<p className="text-[12px] text-gray-500 leading-relaxed">
					Your sourcing request is in. Our team will review it and reach out
					shortly with vendor matches and quotes.
				</p>
			</div>

			{/* Reference ID — only renders if API returned one */}
			{referenceId && (
				<div
				...
				>
					...
					<span className="text-[12px] font-bold text-[#1B3B5F] font-mono">
						{referenceId}
					</span>
				</div>
			)}
			...
			<div
				...
				style={{ background: "#EBF0F8", border: "1px solid #D0DCEA" }}
			>
				<p className="text-[11px] font-bold text-[#1B3B5F] uppercase tracking-wide">
					What happens next
				</p>
				<ol className="space-y-2.5">
					{[
						"Our sourcing team reviews your request within 24 hours.",
						email
							? `Updates and vendor matches will be sent to ${email}.`
							: "Updates and vendor matches will be sent to your email.",
						"Once we have a match, we'll connect you directly with the vendor.",
					].map((stepText, i) => (
						<li
							key={i}
							className="flex items-start gap-2 text-[11px] text-[#1B3B5F]/80 leading-relaxed"
						>
							<span
			...
			<p className="text-center text-[10px] text-gray-400 px-4">
				Need to change something? Reach out at{" "}
				<a
					href="mailto:sourcing@obana.africa"
					className="text-[#1B3B5F] font-medium hover:underline"
				>
					sourcing@obana.africa
				</a>
			</p>
		</div>
	);
...
const SourcingInput = memo(function SourcingInput({
	...
	return (
		<input
			...
			className="w-full px-3 py-2 rounded-lg text-base md:text-[13px] text-[#1B3B5F] outline-none transition-colors placeholder:text-gray-400"
			style={{
				background: bg,
...
const SourcingTextarea = memo(function SourcingTextarea({
	...
	return (
		<textarea
			...
			placeholder={placeholder}
			rows={3}
			className="w-full px-3 py-2 rounded-lg text-base md:text-[13px] text-[#1B3B5F] outline-none resize-none placeholder:text-gray-400"
			style={{
				background: bg,
				border:     `1px solid ${focused ? "#A0B4CB" : "#D9E2EC"}`,
			}}
...
const CurrencyOnlySelector = memo(function CurrencyOnlySelector({
	...
	return (
		<select
			...
			className="w-full px-2 py-1.5 text-base md:text-[11px] font-semibold text-[#1B3B5F] rounded-lg outline-none"
	...
	return (
		...
		>
			...
			<input
				...
				placeholder="0"
				className="flex-1 text-base md:text-[12px] text-right text-[#1B3B5F] bg-transparent outline-none min-w-0"
