...
const CategorySections: React.FC<Props> = ({ onCategoryClick }) => {
	...
	// TEMP: remove once data shape is confirmed
	console.log("[CategorySections]", { isLoading, isError, data });

	const grouped = data?.grouped;
	const isEmpty = !isLoading && !isError && (!grouped || grouped.length === 0);
				...
				{isLoading && (
					<div className="space-y-6">
						{[1, 2].map((i) => (
							<div key={i} className="animate-pulse">
								<div className="h-10 bg-gray-100 rounded-t-xl" />
								<div className="grid grid-cols-4 md:grid-cols-6 gap-3 md:gap-4 p-3 md:p-4 bg-gray-50 rounded-b-xl">
									{[1, 2, 3, 4, 5, 6].map((j) => (
										<div key={j} className="h-[100px] sm:h-36 md:h-[150px] bg-gray-200 rounded-xl" />
									))}
								</div>
							</div>
						))}
